...
SERVICE_NAME="zhenzhufen-web"
PROJECT_NAME="zhenzhufen-website"
...
# 检查必要文件
check_files() {
    local required_files=("Dockerfile" "$COMPOSE_FILE" "nginx.conf" "index.html")
...
# 创建必要目录
create_directories() {
    local directories=("logs/nginx" "logs/proxy" "proxy")
...
build_image() {
    ...
    if $COMPOSE_CMD -f $COMPOSE_FILE build --no-cache --pull; then
        ...
        # 清理相关镜像
        docker rmi $(docker images | grep "zhenzhufen" | awk '{print $3}') 2>/dev/null || true
...
health_check() {
    ...
    if curl -f http://localhost:8080/health &> /dev/null; then
        ...
        print_message $GREEN "网站已成功部署，访问地址: http://localhost:8080"
        print_message $GREEN "生产环境访问地址: https://zhenzhufen.yongli.wang"
    else
...
show_help() {
    ...
    echo "  $0 logs zhenzhufen-web      # 查看主服务日志"
...
main() {
    ...
    print_message $BLUE "珍一润珍珠粉网站 Docker 部署脚本"
    print_message $BLUE "=================================="
