...
// 处理 /start 命令
bot.command("start", (ctx) => ctx.reply("welcome to our translation bot. i will automatically translate messages between russian and chinese."));
...
bot.catch((err) => {
  ...
  if (err.error_code === 409) {
    console.error('❌ 检测到多个机器人实例运行，请确保只有一个实例在运行');
    console.error('💡 解决方案：');
    console.error('   1. 检查是否有其他进程在运行此机器人');
    console.error('   2. 等待几秒钟后重试');
    console.error('   3. 重启服务');
    
    // 等待 5 秒后重试
    setTimeout(() => {
      console.log('🔄 5秒后重试启动机器人...');
...
async function startBot() {
  try {
    ...
    if (error.error_code === 409) {
      console.error('❌ 机器人启动冲突，等待 10 秒后重试...');
